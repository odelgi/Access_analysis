...
if __name__ == '__main__':
    ...
    for llhood in livelihoods:
        ...
        arcpy.AddIndex_management(pellepoints, fields='group{}'.format(llhood), index_name='group') # Add index to speed up querying

        ############# PARALLEL START ###################################################################################
        print('Launch parallel processing')
        tic = time.time()
        fishgroups = range(1, max([row[0] for row in arcpy.da.SearchCursor(fishpoints, 'group{}'.format(llhood))]))
